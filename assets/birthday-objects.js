...
function initBirthdayObjects(){
  ...
  fallbackText.className='birthday-farah-3d';
  fallbackText.textContent='FARAH';
  fallbackText.setAttribute('aria-hidden','true');
  modal.appendChild(fallbackText);
  ...
  renderer.setPixelRatio(Math.min(window.devicePixelRatio||1,1.7));renderer.setClearColor(0x000000,0);renderer.outputColorSpace=THREE.SRGBColorSpace;renderer.toneMapping=THREE.ACESFilmicToneMapping;renderer.toneMappingExposure=1.08;
  const scene=new THREE.Scene();const camera=new THREE.PerspectiveCamera(32,1,.1,30);camera.position.set(0,0,9);
  scene.add(new THREE.HemisphereLight(0xfff3c2,0x211406,1.35));
  const key=new THREE.DirectionalLight(0xfff7dd,3.1);key.position.set(-4,6,8);scene.add(key);
  const fill=new THREE.DirectionalLight(0xe8b93f,1.1);fill.position.set(5,1,4);scene.add(fill);
  const rim=new THREE.DirectionalLight(0xffc83d,1.2);rim.position.set(2,4,-4);scene.add(rim);
  const gold=new THREE.MeshPhysicalMaterial({color:0xe8b93f,roughness:.34,metalness:.08,clearcoat:.42,clearcoatRoughness:.26});
  const sphere=new THREE.Mesh(new THREE.SphereGeometry(.82,48,48),gold.clone());sphere.material.roughness=.28;scene.add(sphere);
  const cube=new THREE.Mesh(new RoundedBoxGeometry(1.45,1.45,1.45,8,.12),gold.clone());cube.rotation.set(.44,-.58,.22);scene.add(cube);
  const pyramid=new THREE.Mesh(new THREE.ConeGeometry(.98,1.72,4,1,false,Math.PI/4),gold.clone());pyramid.rotation.set(-.08,.48,-.08);scene.add(pyramid);
  ...
  const glowA=new THREE.Mesh(new THREE.SphereGeometry(1.03,24,24),glowMaterial);scene.add(glowA);
